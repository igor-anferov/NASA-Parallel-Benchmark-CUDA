...
void exact_rhs()
{
  ...
  for (k = 1; k <= grid_points[2]-2; k++) {
    ...
    for (j = 1; j <= grid_points[1]-2; j++) {
      ...
      for (m = 0; m < 5; m++) {
        i = 1;
        forcing[k][m][j][i] = forcing[k][m][j][i] - dssp *
  ...
  for (j = 1; j <= grid_points[1]-2; j++) {
    ...
    for (i = 1; i <= grid_points[0]-2; i++) {
      ...
      for (k = 1; k <= grid_points[2]-2; k++) {
        ...
        forcing[k][4][j][i] = forcing[k][4][j][i] - tz2 * (
  ...
  for (k = 1; k <= grid_points[2]-2; k++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (m = 0; m < 5; m++) {
          forcing[k][m][j][i] = -1.0 * forcing[k][m][j][i];
        }
      }
    }
  }
